...
check('routine day tap or hold opens day preview', index.includes('function DayPreviewSheet') && index.includes('onOpenDay(d, routine, label)') && index.includes('const beginHold=') && index.includes('onStart={(idx=0)=>') && index.includes('Start workout'));
...
check('check button accepts goal targets without typing them', index.includes('function applyTargetDefaults') && index.includes('if(useTargets) applyTargetDefaults(tk,set)') && index.includes('finishSet(ei,si,e,true)') && index.includes('Tap check to accept goal reps and start rest'));
...
check('workout discard moved into top options sheet', index.includes('function WorkoutOptionsSheet') && index.includes('Workout Options') && index.includes('Lose this active session') && index.includes('workout-options-btn') && !index.includes('className="btn btn-danger btn-sm" onClick={onDiscard}'));
check('guided workout removes current exercise from footer', index.includes('removeCurrentLabel') && index.includes('exercise-danger-zone') && index.includes('confirmRemoveEx(currentIndex)') && index.includes('e.currentEntryIndex=Math.max(0, Math.min(e.entries.length-1, nextCurrent))'));
check('plan day edits live sync into matching active workout', index.includes('mergePlanDayIntoActive') && index.includes('syncActiveWithPlanDay') && index.includes('Plan changes applied to active workout') && index.includes('syncEntryTargetsFromDay') && index.includes('entryHasSessionData'));
check('removed logged plan exercises become session-only', index.includes('sessionOnly:true') && index.includes('if(en.sessionOnly) return;') && index.includes('delete kept.group') && index.includes('sessionOnly: !!(en && en.sessionOnly)') && patch.includes('sessionOnly: !!(en && en.sessionOnly)'));
check('routine day editor preserves supersets and targets', index.includes('const saveDayEdits=()=>{') && index.includes('const rawGroups=day.groups||{}') && index.includes('if(day.targets && day.targets[id]) targets[id]=day.targets[id]') && index.includes('exerciseIds:ids, groups, targets'));
check('post-workout review gates final save', index.includes('function FinishReviewSheet') && index.includes('requestFinishWorkout') && index.includes('completeWorkout') && index.includes('Update routine plan') && index.includes('Save workout'));
check('skip today is separate from remove exercise', index.includes('skipped: !!(en && en.skipped)') && patch.includes('skipped: !!(en && en.skipped)') && index.includes('const skipEx=') && index.includes('Skip today') && index.includes('if(en.skipped) return true'));
check('flat rest default and warmups are available', index.includes('function WarmupSheet') && index.includes('warmupSetsFor') && index.includes('formatRest') && index.includes('startRest = (label, seconds=null)') && index.includes("store.get('restDefault',120)"));
check('progression engine gives next-session guidance', index.includes('function progressionFor') && index.includes('Progress next time') && index.includes('Hold weight') && index.includes('progression-card'));
check('structured machine setup fields are available', index.includes('const MACHINE_KEYS') && index.includes('parseMachineSettings') && index.includes('formatMachineSettings') && index.includes('machine-grid'));
check('training command bar and update polish exist', index.includes('train-command-bar') && index.includes('sync-status-pill') && index.includes('update-banner') && index.includes('ironlog:update-ready'));
...
check('rest timer can schedule phone notifications', index.includes('requestRestNotificationPermission') && index.includes('SCHEDULE_REST_NOTIFICATION') && index.includes('Enable phone alerts') && index.includes('wakeLock') && sw.includes('showNotification') && sw.includes('notificationclick'));
...
check('reorder bottom space and long press no-select are protected', index.includes('--bottom-safe') && index.includes('scroll-padding-bottom') && index.includes('reorder-sheet .sheet-body{padding-bottom:calc(260px') && index.includes('document.body.style.userSelect') && index.includes('onSelectStart:(e)=>e.preventDefault()'));

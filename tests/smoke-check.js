...
check('live routine edits persist to day template', index.includes('persistRoutineDayShape') && index.includes('routineShapeFromEntries') && index.includes('targets:shape.targets') && index.includes('onRoutineUpdate={persistRoutineDayShape}') && index.includes('saveActive(e, true)'));
...
check('rest timer can schedule phone notifications', index.includes('requestRestNotificationPermission') && index.includes('SCHEDULE_REST_NOTIFICATION') && index.includes('Enable phone alerts') && sw.includes('showNotification') && sw.includes('notificationclick') && sw.includes('ironlog-v45'));
